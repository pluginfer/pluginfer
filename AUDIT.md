...
| 4 | Untrusted-node output unverifiable on the mesh | **MITIGATED + ECONOMIC** — `core/quorum_verify.py`: K-of-N redundant execution + agreement; single liar outvoted and unpaid; disputes surfaced. Now LIVE in the buyer path (`payload={"quorum_n": N}` → quorum-replicate consortium) and backed by the economic layer (`core/economic_layer.py`): providers bond stake, a dissenter proved wrong by an accepted majority is slashed to the honest voters, disputes refund the buyer in full and slash nobody. Wiring this surfaced and fixed three live bugs: byzantine dissenters were still being PAID (settlement ignored the dissenter list), a split-brain billed the buyer as partial-success with no result, and dissenter attribution could land on the wrong provider. Still defeats independent faults, **not** a colluding majority — the bond changes the economics of attempting it, not the math. Opt-in per job (cost = N× compute) |
